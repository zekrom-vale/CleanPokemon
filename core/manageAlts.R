...
# Should change Necrozma to Necrozma and move type to Class

Necrozma=alts.m%>%
	filter(
		str_detect(Name,"Necrozma")
	)%>%
	extract(
		Name,
		into=c("Class","Name"),
		...
		Classification=if_else(Classification=="Normal",Class,paste(Classification,Class)),
		Class=NULL
	);
# Add Normal Necrozma
Necrozma=Necrozma%>%
...
# Now just junk that I did not remove!
...
# Remove junk
rm(
	varations, varations.f, varations.m, alts, alts.f, alts.m, alts.miss, types, types.f, types.f.f, Necrozma
);
